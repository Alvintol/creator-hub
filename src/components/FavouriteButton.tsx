import type React from "react";
import { useHubActions, useHubState, type FavoritesState } from "../providers/HubProvider";

type FavouriteKind = "creator" | "listing";

type HeartProps = {
  filled: boolean;
};

type FavouriteButtonProps = {
  kind: FavouriteKind;
  idOrHandle: string;
  className?: string;
};

const classes = {
  heart: "h-5 w-5",

  btnBase: "inline-flex items-center justify-center rounded-xl border px-2 py-2",
  btnOn: "border-rose-200 bg-rose-50 text-rose-600",
  btnOff: "border-zinc-200 bg-white text-zinc-700 hover:bg-zinc-50",
} as const;

const Heart = (props: HeartProps) => {
  const { filled } = props;

  return (
    <svg viewBox="0 0 24 24" className={classes.heart} aria-hidden="true">
      <path
        d="M12 21s-7.2-4.7-9.6-9.2C.7 8.4 2.4 5.6 5.5 5.1c1.7-.3 3.3.4 4.4 1.6 1.1-1.2 2.7-1.9 4.4-1.6 3.1.5 4.8 3.3 3.1 6.7C19.2 16.3 12 21 12 21z"
        fill={filled ? "currentColor" : "none"}
        stroke="currentColor"
        strokeWidth="1.6"
      />
    </svg>
  );
};

const getIsFav = (kind: FavouriteKind, idOrHandle: string, favorites: FavoritesState): boolean => {
  if (kind === "creator") return !!favorites.creators[idOrHandle];
  return !!favorites.listings[idOrHandle];
}

const getButtonClassName = (isFav: boolean, className: string): string => {
  const variant = isFav ? classes.btnOn : classes.btnOff;
  return `${classes.btnBase} ${variant} ${className}`.trim();
}

const FavouriteButton = (props: FavouriteButtonProps) => {
  const { kind, idOrHandle, className = "" } = props;

  const { favorites } = useHubState();
  const { toggleFavoriteCreator, toggleFavoriteListing } = useHubActions();

  const isFav = getIsFav(kind, idOrHandle, favorites);

  const onClick: React.MouseEventHandler<HTMLButtonElement> = (e) => {
    e.preventDefault();
    e.stopPropagation();

    if (kind === "creator") toggleFavoriteCreator(idOrHandle);
    else toggleFavoriteListing(idOrHandle);
  };

  return (
    <button
      onClick={onClick}
      className={getButtonClassName(isFav, className)}
      aria-label={isFav ? "Remove favorite" : "Add favorite"}
      title={isFav ? "Favorited" : "Favorite"}
      type="button"
    >
      <Heart filled={isFav} />
    </button>
  );
};

export default FavouriteButton;