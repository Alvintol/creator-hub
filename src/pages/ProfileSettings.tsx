...
const classes = {
  ...
  textarea:
    "w-full rounded-xl bg-white px-4 py-3 text-sm outline-none transition ring-1 ring-zinc-300/70 focus-visible:ring-2 focus-visible:ring-zinc-400/70 focus-visible:outline-none",

  row: "mt-5 flex flex-wrap items-center gap-3",
  btnPrimary: "btnPrimary",
  btnOutline: "btnOutline",

  bannerOk: "card p-4 border border-emerald-200 bg-emerald-50 text-emerald-900",
  bannerErr: "card p-4 border-rose-200 bg-rose-50 text-rose-900",
...
const isValidHandle = (v: string): boolean => {
  // 3–25 chars, letters/numbers/_/-, must start with letter/number
  return /^[a-z0-9][a-z0-9_-]{2,24}$/i.test(v);
};
...
const ProfileSettings = () => {
  ...
  // show twitch callback banners if we redirected back here
  useEffect(() => {
    const p = new URLSearchParams(search);
    const twitch = p.get("twitch");
    if (!twitch) return;

    if (twitch === "connected") {
      const ageOk = p.get("age_ok") === "1";
      setOkMsg(
        ageOk
    ...
    if (twitch === "error") {
      const msg = p.get("msg") ?? "Twitch connect failed.";
      setErrMsg(msg);
      setOkMsg(null);
    }

    // remove query params so banner doesn't reappear on refresh
  ...
  const twitchStatus = useMemo(() => {
    if (!profile?.twitch_login) return "Not connected";
    if (profile.twitch_age_ok) return "Connected (eligible)";
    return "Connected (account too new)";
  }, [profile]);

  const canEnableCreator = useMemo(() => {
    return !!profile?.twitch_age_ok;
  }, [profile]);

  const myProfileLink = useMemo(() => {
    const h = (profile?.handle ?? "").trim();
    return h ? `/creator/${h}` : "/creators";
  }, [profile]);
  ...
  const onSave = async () => {
    ...
    try {
      ...
      // if they manually set a display name, stop auto-syncing from email
      if ((profile?.display_name_auto ?? true) && dn) {
  ...
  const onToggleCreator = async () => {
    ...
    const next = !profile.creator_enabled;
    ...
    try {
      ...
      const { error: updErr } = await supabase
        .from("profiles")
        .update({ creator_enabled: next })
        .eq("user_id", user.id);

      if (updErr) throw updErr;

      setOkMsg(next ? "Creator mode enabled." : "Creator mode disabled.");
      await refetch();
    } catch (e) {
      // If twitch_age_ok gating is active, this will fail with an RLS error.
      const msg = canEnableCreator
        ? getErrorMessage(e)
  ...
  const onConnectTwitch = async () => {
    ...
    try {
      ...
      if (!r.ok || !json.url) {
        throw new Error(json?.error || `Twitch connect failed (${r.status})`);
      }
    ...
    <div className={classes.page}>
      ...
      <div className={classes.card}>
        ...
        <div className={classes.grid}>
          ...
          <div className={`${classes.field} md:col-span-2`}>
        ...
        <div className={classes.toggleRow}>
          <div>
            ...
            <div className={classes.toggleDesc}>
              Twitch: <span className="font-semibold">{twitchStatus}</span>
              {profile?.twitch_login ? (
                <span className="text-zinc-400"> • @{profile.twitch_login}</span>
              ) : null}
            </div>
