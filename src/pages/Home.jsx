...
import { CATEGORIES, VIDEO_SUBTYPES } from "../domain/catalog";
        ...
        <div className="space-y-10">
            ...
            <section className="space-y-3">
                <div className="flex items-baseline justify-between gap-4">
                    <h2 className="text-xl font-extrabold tracking-tight">Browse categories</h2>
                            ...
                            {cat.key === "video-editing" && (
                                <div className="mt-3 flex flex-wrap gap-2">
                                    {VIDEO_SUBTYPES.map((v) => (
                                        <Link
                                            key={v.key}
                                            to={`/market?cat=video-editing&video=${v.key}`}
                            ...
                            >
                                ...
                                <div className="p-4">
                                    <div className="flex items-start justify-between gap-3">
                                        ...
                                        <div className="shrink-0 text-sm font-extrabold">{priceText(l)}</div>
                                    </div>

                                    <p className="mt-2 line-clamp-2 text-sm text-zinc-600">{l.short}</p>
            ...
            <section className="space-y-3">
                <div className="flex items-baseline justify-between gap-4">
                    ...
                    <Link to="/live" className="text-sm font-semibold text-zinc-600 hover:text-zinc-900">
                        View all →
                    </Link>
                </div>
                        ...
                        {liveNow.slice(0, 6).map((c) => (
                            ...
                            >
                                <div className="flex flex-wrap items-center gap-2">
                                    <div className="text-base font-extrabold tracking-tight">{c.displayName}</div>
                                    {c.verified && (
                                        <span className="rounded-full border border-zinc-200 bg-white px-2 py-0.5 text-xs font-semibold">
                                            Verified
                                        </span>
                                ...
                                <p className="mt-2 text-xs text-zinc-500">Platform: {c.live?.platform}</p>
                            </Link>
                        ))}
                    </div>
                )}
